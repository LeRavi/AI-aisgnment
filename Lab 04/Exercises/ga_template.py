...
def reproduce(mother, father):
    ...
    child = (parent[random.randint(0, 1)][0], parent[random.randint(0, 1)][1], parent[random.randint(0, 1)][2])
    return child
...
def mutate(individual):
    ...
    """
    mutate_individual = list(individual)
    number = random.randint(0, 2)

    if mutate_individual[number] == 1:
        mutate_individual[number] = 0
    else:
        mutate_individual[number] = 1
    return tuple(mutate_individual)
...
def random_selection(population, fitness_fn):
    ...
    temporary = list(fitness)
    temporary.sort()
    length = len(temporary)
    print(temporary)
    first = temporary[length - 1]
    second = temporary[length - 2]
    first_index = fitness.index(first)
    second_index = fitness.index(second)

    selected = ordered_population[first_index], ordered_population[second_index]
    return selected
